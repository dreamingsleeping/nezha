...
install_agent() {
    ...
    echo -e "正在下载监控端"
    wget -t 2 -T 300 -O nezha-agent_linux_${os_arch}.zip https://${GITHUB_URL}/nezhahq/agent/releases/download/${version}/nezha-agent_linux_${os_arch}.zip >/dev/null 2>&1
    if [[ $? != 0 ]]; then
...
modify_agent_config() {
    ...
    if [ "$os_alpine" != 1 ]; then
        wget -t 2 -T 300 -O $NZ_AGENT_SERVICE https://${GITHUB_RAW_URL}/script/nezha-agent.service >/dev/null 2>&1
        if [[ $? != 0 ]]; then
            echo -e "${red}文件下载失败，请检查本机能否连接 ${GITHUB_RAW_URL}${plain}"
            return 0
        fi
    else
        wget -t 2 -T 300 -O $NZ_AGENT_SERVICERC https://${GITHUB_RAW_URL}/script/nezha-agent >/dev/null 2>&1
...
modify_dashboard_config() {
    ...
    if [[ $IS_DOCKER_NEZHA == 1 ]]; then
        ...
        wget -t 2 -T 300 -O /tmp/nezha-docker-compose.yaml https://${GITHUB_RAW_URL}/script/docker-compose.yaml >/dev/null 2>&1
        if [[ $? != 0 ]]; then
    ...
    wget -t 2 -T 300 -O /tmp/nezha-config.yaml https://${GITHUB_RAW_URL}/script/config.yaml >/dev/null 2>&1
    if [[ $? != 0 ]]; then
        echo -e "${red}下载脚本失败，请检查本机能否连接 ${GITHUB_RAW_URL}${plain}"
        return 0
    fi
    ...
    if [[ $IS_DOCKER_NEZHA == 0 ]]; then
        echo -e "正在下载服务文件"
        if [ "$os_alpine" != 1 ]; then
            wget -t 2 -T 300 -O $NZ_DASHBOARD_SERVICE https://${GITHUB_RAW_URL}/script/nezha-dashboard.service >/dev/null 2>&1
        else
            wget -t 2 -T 300 -O $NZ_DASHBOARD_SERVICERC https://${GITHUB_RAW_URL}/script/nezha-dashboard >/dev/null 2>&1
